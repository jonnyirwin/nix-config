...
#     (`catppuccin.tmux.enable`), so it follows the global
#     `catppuccin.flavor` set in home.nix rather than a `@catppuccin_flavour`
#     line in a hand-written conf.
#   * The accent colour is derived from `catppuccin.accent` (home.nix)
#     instead of being rewritten in place by ~/.dotfiles/catppuccin/.local/bin/set-accent.
#     To change it: edit `catppuccin.accent` and rebuild.
#
...
let
  # Catppuccin Mocha accent palette. The catppuccin/nix module exposes
  # `catppuccin.accent` as a name only, not a hex value, so we map it here.
  # Keep in sync with the palette in ~/.dotfiles/catppuccin/.local/bin/set-accent.
  accentHex = {
    rosewater = "#f5e0dc";
    flamingo  = "#f2cdcd";
    pink      = "#f5c2e7";
